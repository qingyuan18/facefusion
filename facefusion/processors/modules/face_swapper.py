...
def create_face_by_base64(base64_data: str) -> Face:
	"""Create a Face object from base64 encoded image data."""
	import base64
	import cv2
	import io
	...
	vision_frame = cv2.cvtColor(numpy.array(image), cv2.COLOR_RGB2BGR)
	...
	if faces:
		...
		raise ValueError("No face detected in the provided base64 image data")


def find_most_similar_face(target_faces: List[Face], reference_face: Face, similarity_threshold: float = 0.3) -> Optional[Face]:
	"""
...
def process_frames(source_paths : List[str], queue_payloads : List[QueuePayload], update_progress : UpdateProgress) -> None:
	...
	if faces_mapping_json:
		...
		for source_frame in source_frames:
			...
			if temp_faces:
				source_faces_inputs.append(get_first(temp_faces))

		# Process each frame with multi-face mapping
		for queue_payload in process_manager.manage(queue_payloads):
			target_vision_path = queue_payload['frame_path']
			target_vision_frame = read_image(target_vision_path)

			# Get reference faces for this frame
			...
			for index, source_face in enumerate(source_faces_inputs):
				if str(index) in faces_mapping_dict:
					...
					if most_similar_face:
						...
						# Process frame with the specific face to swap
						target_vision_frame = process_frame(
						{
							'reference_faces': reference_faces_for_swap,
		...
		# Original logic for non-many mode
		reference_faces = get_reference_faces() if 'reference' in state_manager.get_item('face_selector_mode') else None
		source_frames = read_static_images(source_paths)
		source_faces = []
		...
		for index, source_face in enumerate(source_faces_inputs):
			if str(index) in faces_mapping_dict:
				...
				if most_similar_face:
					...
					# Process image with the specific face to swap
					target_vision_frame = process_frame(
					{
						'reference_faces': reference_faces_for_swap,
						'source_face': source_face,
